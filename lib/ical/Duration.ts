...
export class Duration implements DurationData {
   ...
   */
  constructor(data?: Partial<DurationData>) {
    this.fromData(data)
  }
   ...
   */
  readonly icaltype: string = 'duration'
   ...
   */
  fromData(aData: Partial<DurationData> = {}) {
 ...
 * @param dict target to assign values to
 */
function parseDurationChunk(letter: string, number: string, object: DurationData) {
  let type: undefined | keyof DurationData
  switch (letter) {
    case 'P':
      if (number && number === '-') {
