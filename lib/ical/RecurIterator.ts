...
import { formatClassType, trunc, clone } from './helpers'
...
export class RecurIterator {
   ...
   *
   * @param options                The iterator options
   * @param options.rule       The rule to iterate.
   * @param options.dtstart     The start date of the event.
   ...
   */
  _byDayAndMonthDay(isInit?: boolean) {
  ...
  next_weekday_by_week() {
    ...
    for (;;) {
      ...
      const next = Time.fromDayOfYear(startOfWeek + dow,
                                                this.last.year)
  ...
  increment_generic(inc, aDateAttr,
