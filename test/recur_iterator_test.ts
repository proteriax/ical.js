...
describe('recur_iterator', function () {
  ...
  function datesBetween(start, end, includeTime?: boolean) {
    const list: Date[] = []
  ...
  describe('#toJSON', function () {
    ...
    const recur = ICAL.Recur.fromString('FREQ=MONTHLY;COUNT=12;INTERVAL=3')
    ...
    it('completed', function () {
      ...
      expect(newIter.next(), 'new iter next').to.be.null
    ...
    it('INTERVAL: mid iteration (two iterations)', function () {
      ...
      while (inc++ < 8) {
        expect(
          iterator.next()!.toJSDate()).to.deep.equal(
    ...
    it('from the begining of iteration', function () {
      ...
      while (inc++ < 10) {
        expect(
          newIter.next()!.toJSDate()).to.equal(
  ...
  describe('#normalizeByMonthDayRules', function () {
    const start = ICAL.Time.fromString('2012-02-01T09:00:00')
    const recur = ICAL.Recur.fromString('FREQ=MONTHLY;COUNT=2')
    const iterator = recur.iterator(start)
